    ...
    <ScrollView style={styles.screen}>
      <View style={styles.header}>
        <View style={styles.avatar}>
          <Text style={styles.avatarInitial}>{(user?.name ?? user?.email ?? "D")[0].toUpperCase()}</Text>
        </View>
      ...
      <View style={styles.menu}>
        {[
          ...
          { icon: "💰", label: "Earnings" },
          { icon: "🏦", label: "UPI / Payment Settings" },
          { icon: "❓", label: "Support" },
        ].map(({ icon, label }) => (
...
const styles = StyleSheet.create({
  ...
  avatar: {
    ...
    backgroundColor: "#3b82f6", alignItems: "center", justifyContent: "center", marginBottom: 12,
  },
